...
def plot_seed_depth():
    ...
    cldepths = list(map(lambda d: len([cl for cl,feat in langdict.items()
                                       if (("CLDepth" in langdict[cl])
                                       and (langdict[cl]["CLDepth"] == d))
                                       and ((("GitSeed" in langdict[cl]) and (langdict[cl]["GitSeed"] == 1))
                                            or ("TIOBE" in feat and (langdict[cl]["TIOBE"] == 1)))])
                        , range(5)))
    cffdepths = list(map(lambda d: len([cl for cl,feat in langdict.items()
                                       if (("CFFDepth" in langdict[cl])
                                       and (langdict[cl]["CFFDepth"] == d))
                                       and ((("GitSeed" in langdict[cl]) and (langdict[cl]["GitSeed"] == 1))
                                            or ("TIOBE" in feat and (langdict[cl]["TIOBE"] == 1)))])
                         , range(5)))
    ...
    df = read_csv(StringIO(csvtext), delimiter=',', names=["depth", "#CL", "#CFF"],
                  dtype={'depth': int,
                         ...
                         '#CFF': int})
    print(df)
    ax = df.plot(x="depth", y=["#CL", "#CFF"], kind="bar", logy=True, width=0.8)
