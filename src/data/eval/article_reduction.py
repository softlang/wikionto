...
langdict = load(f)
df = DataFrame(columns=['#seed','#art','#rec_art','#rec_nonseed','reduced %'], index=['Formal languages','Computer file formats','Installation software'])
for c in CATS:
    ...
    df.loc[c.replace("Category:", "").replace("_"," ")] = Series({'#art': len(articles), '#seed': len(seed_articles),
