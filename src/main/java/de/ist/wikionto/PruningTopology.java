		...
		while (rs.hasNext()) {
			...
			if (log.contains(t + " having top classifiers " + top1 + " & " + top2 + " is valid")) {
				...
				if (r == 1) {
					...
					if (QueryUtil.getPathFromClassToClass(dataset, "Computer languages", t).isEmpty()) {
						String rep = "INSERT { ?t <http://myWikiTax.de/hasSubclassifier> ?r . }"
								+ "\nWHERE{ ?t <http://myWikiTax.de/name> \"" + t + "\" ."
								+ "\n ?r <http://myWikiTax.de/name> \"Computer languages\" .}";
		...
		while (rs.hasNext()) {
			...
			if (log.contains(dri + " having top classifiers " + top1 + " & " + top2 + " is valid")) {
				...
				if (r == 1) {
					...
					List<String> cats1 = map.get("Path from " + top1);
					cats1.remove(cats1.size() - 1);
					List<String> cats2 = map.get("Path from " + top2);
