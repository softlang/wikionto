 ...
 * @author Marcel
 */
public class WikiTaxToJenaTDB2 {
	...
	private static void transformLinks(Classifier classifier) {
		...
		for (String link : links) {
			if (link.contains("Category:")) {
				...
				c.setName(link);
				transformLink(resource, c);
			} else {
				Instance i = new Instance();
				i.setName(link);
				transformLink(resource, i);
			}
	...
	private static void transformLink(Resource resource, Instance link) {
		Resource linkResource;
		if (!instanceResMap.containsKey(link.getURIName())) {
			linkResource = model.createResource(iURI + instanceResMap.size());
			instanceResMap.put(link.getURIName(), linkResource);
			linkResource.addProperty(nameP, link.getName());

		} else {
			linkResource = instanceResMap.get(link.getURIName());

		}
		resource.addProperty(linkP, linkResource);

	}

	private static void transformLink(Resource resource, Classifier link) {
		Resource linkResource;
		if (!classResMap.containsKey(link.getURIName())) {
			linkResource = model.createResource(cURI + classResMap.size());
			classResMap.put(link.getURIName(), linkResource);
			linkResource.addProperty(nameP, link.getName());

		} else {
			linkResource = classResMap.get(link.getURIName());
