...
public class DissolveGUI extends JFrame {
	...
	private void getInstances() {
		...
		try {
			...
			System.err.println("IOException at getInstances() while reading query file.");
			ex.printStackTrace();
		}
		String queryString = "";
	...
	private void getSubtypes() {
		...
		try {
			...
			System.err.println("IOException at getSubtypes() while reading query file.");
			ex.printStackTrace();
		}
		String queryString = "";
		for (String line : lines) {
	...
	private void collapseType() {
		...
		for(int j = 0; j < instances.length;j++){
			if(ibuttons[j].isSelected()){
				prune.abandonInstance(instances[j]);
		...
		for(int j = 0; j < subtypes.length;j++){
			if(sbuttons[j].isSelected()){
				prune.abandonClassifier(subtypes[j]);
		...
		for(int j = 0; j<subtypes.length;j++){
			if(rsbuttons[j].isSelected()){
				prune.removeSubclassifier(typename, subtypes[j]);
	...
	public static void main(String[] args) {
		Dataset dataset = TDBFactory.createDataset("./Computerlanguages");
